...
const register = async (req, res, next) => {
  try {
    const { name, email, phone, password, role, plan, dni, acceptedTyc, acceptedPrivacy, confirmedAge } = req.body;

    if (!name || !email || !password) {
      return res.status(400).json({ error: 'Nombre, email y contraseña son obligatorios' });
    }

    const result = await authService.register({ name, email, phone, password, role, plan, dni, acceptedTyc, acceptedPrivacy, confirmedAge });
